...
% These connection links between nodes are represented as the \(M \times M\) network connection matrix \(\mtxb{C}\) with its elements defined as \(\mathrm{G}_{ij} = 1\) if the \(i\)-th node shares its signal information with the \(j\)-th node and \(\mathrm{G}_{ij} = 0\) otherwise.
...
\begin{figure}
    \centering
    \includestandalone[trim={0 0 0 0},clip,height=2.5cm]{tikz/topology_general}
    % \vspace*{-0.3cm}
    \caption{WSN}
...
\(\mathcal{G}(i,j)\) denotes the mapping of \(L\) local variable components \((\wf_i)_j\), i.e. one of the frequency responses in the stacked vector, to the corresponding global variable components \((\hf)_g\).
