...
\cite{bertrandApplicationsTrendsWireless2011b}
\begin{todo}
    References
\end{todo}
...
This is used to solve a BSI problem of the same form as in \eqref{eq:frequency_domain:min_prob}, which however only involves the subset of channels.
...
% The global variable \(\hf\), a stacked vector of the \(M\) single-channel consensus variables \(\hf_i\), is updated by a distributed update.
...
The choice to present the median is based on the fact that randomly generated impulse responses may have close common zeros diminishing identifyability (for all algorithms) \cite{naylorNearCommonZerosBlind2008}, here then considered outliers.
...
\autoref{fig:perf_eval:NPM_over_time_exp2} shows the median of 30 Monte-Carlo runs for the two setups, which suggests a proportional relation of convergence speed, channel number \(M\) and sub-problem overlap \(\zeta\) while the steady-state error appears not dependent on \(\zeta\).
\begin{attention}
    Some plots and values will change slightly.
\end{attention}
...
Each node in a WSN solves a BSI sub-problem estimating channel responses using signal data from neighboring nodes to form the cross-relation problem with a subset of channels.
Through 
\begin{todo}
    FINISH
\end{todo}
%  while maintaining steady-state error performance and convergence speed.
