...
We define a set \(\Tset_i \subseteq \Mset\) for each node \(i\) with \(i \in \Mset\), which contains the indices of the set of nodes which signal \(\x_i(n)\) is transmitted to.
...
The parameter \(\zeta\) is defined as the ratio of randomly selected connections to possible connections (excluding the ring topology). Topologies satisfying this definition for \(\zeta \in \{0.0,\,0.25,\,0.5,\,0.75\}\) were generated.
