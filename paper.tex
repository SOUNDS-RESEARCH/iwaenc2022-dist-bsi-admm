...
\begin{abstract}
    ...
    The network consists of a fixed number of nodes each equipped with a processing unit as well as a sensor that represents an output channel of the SIMO system and a consensus fusion center.
...
\begin{todo}
    is this even worth a subsection?
\end{todo}

We consider a WSN consisting of \(M\) sensor nodes and a consensus fusion center where each node acquires an output signal \(\x_i(n)\).
We define a set \(\Tset_i\) for each node \(i\) with \(i \in \Mset\), which contains the indices of all nodes which have access to the signals \(\x_i(n)\), i.e. the signal information is transmitted to.
Note that \(i \in \Tset_i\) for the purpose of simplifying notation in later sections, but evidently, no actual transmission is necessary in that case.
Further, we define a set \(\Rset_i\) for each node with \(i \in \Mset\), which contains all indices of nodes which the node receives from.
Furthermore, all nodes exchange information on the consensus with the fusion center.
Network topologies with fusion centers come with the drawback of introducing the necessity of nodes transmitting additional data, however, the task of mitigating this will be addressed in future work.
...
\begin{figure}
    \centering
    \subfloat[][]{\includestandalone[trim={0 0 0 0},clip,width=0.23\columnwidth]{tikz/topology_general}\label{fig:sensor_network:WSN}}\hspace*{0.02\columnwidth}
    \subfloat[][]{\includegraphics[trim={0.3cm 0.25cm 1.7cm 0.0cm},clip,width=0.75\columnwidth]{images/parameter_mapping2.pdf}\label{fig:problem_splitting:parameter_mapping}}
    \vspace*{-0.1cm}
    \caption{(a) shows a minimal example of a network topology with 4 nodes (\(\circ\)) and consensus fusion center (\(\diamond\)). Black and grey arrows indicate exchange of signal information and consensus information respectively. (b) shows the equivalent parameter mapping of the local variable components \(({\wf}_i)_j\) to the global consensus variable components \(({\hf})_j\) via \(\mathcal{G}(i,j)\) for the network in (a).}
...
\begin{align}
    \operatorname{minimize} \quad &\sum_{i \in \Mset} \tilde{J}_i(\wf_i)\\
...
\begin{align}
    \mathcal{L}_{\rho} (\wf,\hf,\uuf) &= \sum_{i \in \Mset} \left( \wf_i^\herm \hat{\aRhof}_i \wf_i + 2 \Re \left( \uuf_i^{\herm} \left(\wf_i - \tilde{\hf}_i\right) \right)\right. \vphantom{+ \rho \left\| \wf_i - \tilde{\hf}_i \right\|} \nonumber\\
...
The implicit overlap-save matrices in \(\bm{{V}}_i^m\) make this a constrained update step which is costly to compute, so in order to reduce complexity, we introduce the approximation
\begin{equation}
...
\begin{todo}
    Here could be added that the consensus update step doesnt have to be done each time step which would reduce the number of transmissions (obviously slows convergence). But that would be a claim that is not backed by convergence proof, only simulation results.
\end{todo}
